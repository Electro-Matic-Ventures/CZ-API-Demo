...
typedef struct
{
	INT8U ElectricLock;											//0x00上锁,0x01开锁(默认电子锁是锁上的)
	INT8U Generator;											//0x00关闭,0x01打开
	INT8U Camera;												//0x00关闭,0x01打开
	INT8U Rev2;
	INT8U Rev[4];
}EXTEQUIPMENT_ST;
...
//================================================================================================================
//==== 模块配置文件,模块数据部分，实际的模块文件是由文件头加多个模块数据部分组成,共192Bytes
//================================================================================================================
typedef struct
{
	INT16U synCode;												//同步头
	...
	INT16U CRC;													//特征值
	INT16U Bright;												//最大亮度	
	INT8U  driverSN[12];										//驱动板序列号
	INT8U  driverType[12];										//驱动板型号
	INT8U  pointNumX;											//驱动板横向点数
	INT8U  pointNumY;											//驱动板纵向点数
	INT8U  phase595;											//595时序
	INT8U  num595;												//595个数
	INT8U  driverIC[12];										//驱动IC
	INT8U  ledType[12];											//LED管型号
	INT16U phaseScan;											//时序
	INT16U redRes;												//红管限流电阻
	INT16U greenRes;											//绿管限流电阻
	INT16U blueRes;												//蓝管限流电阻
	INT16U pitch;												//点间距

	INT16U EepSpace;											//红色X色坐标
	INT16U BRCRC;												//红色Y色坐标
	INT16U Rev1;												//红色亮度值
	INT8U  BRTable[10];											//绿色X色坐标
	INT8U  Rev2[6];												//绿色Y色坐标
	INT16U functions;											//支持的特性
	INT16U RTCRC;												//热敏电阻特征码
	INT8U  RTTable[32];											//热敏电阻

	INT8U  IGain_Enable;
	INT8U  RedGain;
	INT8U  RedIGain_Half;
	INT8U  GreenGain;
	INT8U  GreenIGain_Half;
	INT8U  BlueGain;
	INT8U  BlueIGain_Half;
	INT8U  WhiteGain;
	INT8U  WhiteIGain_Half;
	INT8U  rev[51];												//保留 

	INT8U  REVEPROM[4];											//下面用
}MODULE_CFG_DATA;
...
typedef struct
{
	INT32U FileMask;											//新雷达日志文件标志, 'RLOG'=0x474F4C52
	INT32U RadarLogNum;											//日志中存在的Radar条数
	INT32U RadarWriteloc;										//最近一次日志写入偏移坐标
	INT16U RadarLastDate;          								//最近一次Radar日志写入的日期	
	INT16U RadarLastTime;										//最近一次Radar日志写入的日期
}NEWRARAD_EXLOGHEAD_STRUCT;

typedef  struct
{
	INT16U RadarSpeed;											//雷达速度
	INT16U CarLen;												//车的长度
	INT16U RadarDate;											//Radar日志写入的日期	
	INT16U RadarTime;											//Radar日志写入的日期
}NEWRARAD_EXLOG_STRUCT;
